...
findcfilesrecursive = $(shell find $1 -name '*.m' -o -name '*.mm' -o -name '*.c' -o -name '*.cc')
findjavafiles = $(call find,$1,-iname '*.java')
filterout = $(foreach s,$2,$(if $(findstring $1,$s),,$s))
findhdirs = $(sort $(dir $(shell find $1 -name '*.h')))
incdirs = $(foreach d,$(call findhdirs,$1),'-I$d')
# filtercfiles,paths
...
checkpath = $(if $(wildcard $1),$(firstword $(wildcard $1)),$(error $(if $2,$2,Path $1 does not exist)))
